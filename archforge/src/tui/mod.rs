...
impl App {
    pub fn new() -> Self {
        ...
        app.packages = get_sample_packages();
        app.status_message = "Welcome to ArchForge! Press ? for help".to_string();
        app
    }
    ...
    fn execute_command(&mut self, cmd: &str) {
        ...
        match parts[0] {
            ...
            "w" | "write" => {
                self.status_message = "PKGBUILD saved!".to_string();
            }
            "wq" | "x" => {
                self.status_message = "Saved and quit".to_string();
                self.should_quit = true;
            }
            "help" | "h" => {
                self.status_message = "Commands: w=write, q=quit, :wq=save+quit".to_string();
            }
            "" => {}
            _ => {
...
fn render_deploy_tab(f: &mut Frame, app: &App, area: Rect) {
    let options = vec![
        "1. Deploy to AUR",
